...
def getVote(x):
    d = {'num': len(x["vote"]), 'userId': x["user_id"],
                    'editCount': x["edit_count"]}
    return d
...
def check(teamId: str) -> str:
    ...
    with conn.cursor() as cursor:
        ...
        if len(result["completed"]) == 1:
            typ = 'check'
            sql = 'Select * from project.schedule_version where edit_Count =%s'
            ...
            return 'htfucomehere'
        # 投票完或表態完?
        cursor.execute(sql, (result["completed"][0]))
        res = cursor.fetchall()
        votes = list(map(getVote, res))
        voteSum = 0
        re = dict()
        re["type"] = typ
        ...
        #如果有人沒投完
            ...
            # return 誰還沒有投票/ 表態
            votedUser = list(map(getVotedUsers,res))
            notYet = list(filter(lambda e: e not in votedUser, result['teammate_list'].split(",")))
            return notYet
